...
void testG1Z8KernelFP16_Execute(id<MTLDevice> device, id<MTLCommandQueue> queue,
                       int batch, int height, int width, int inChannels, int outChannels,
                       int numRuns, std::vector<long>& durations) {
    NSError* error = nil;
...
//                                                  error:&error];
    ...
    // 准备命令缓冲区数组
    std::vector<id<MTLCommandBuffer>> commandBuffers(numRuns);
    
    auto start = std::chrono::high_resolution_clock::now();
    
    // 创建并提交所有命令缓冲区（但不等待完成）
    for (int i = 0; i < numRuns; i++) {
        commandBuffers[i] = [queue commandBuffer];
        id<MTLComputeCommandEncoder> encoder = [commandBuffers[i] computeCommandEncoder];
...
// 修改后的测试函数（以testG1Z8KernelFP16为例）
void testGemm32x16KernelFP16_Execute(id<MTLDevice> device, id<MTLCommandQueue> queue,
                       int batch, int height, int width, int inChannels, int outChannels,
                       int numRuns, std::vector<long>& durations) {
    NSError* error = nil;
...
void Run_Execute() {
    ...
    // 存储每次运行的时间
    std::vector<long> g1z8Durations;
    std::vector<long> gemmDurations;
    
    // 预热运行
    testG1Z8KernelFP16_Execute(device, queue, batch, height, width, inChannels, outChannels, 1, g1z8Durations);
    testGemm32x16KernelFP16_Execute(device, queue, batch, height, width, inChannels, outChannels, 1, gemmDurations);
    g1z8Durations.clear();
    gemmDurations.clear();
    // 正式测试运行
    testG1Z8KernelFP16_Execute(device, queue, batch, height, width, inChannels, outChannels, numRuns, g1z8Durations);
    testGemm32x16KernelFP16_Execute(device, queue, batch, height, width, inChannels, outChannels, numRuns, gemmDurations);
    
    // 计算统计信息
    long g1z8Total = 0, gemmTotal = 0;
    long g1z8Min = LONG_MAX, gemmMin = LONG_MAX;
    long g1z8Max = 0, gemmMax = 0;
    
    int iTest1 = g1z8Durations.size();
    int iTest2 =  gemmDurations.size();
    std::cout<<"  1: "<< iTest1<< "  "<<iTest2<<std::endl;
    
    // 跳过预热运行的数据（索引0）
    for (int i = 0; i < g1z8Durations.size(); i++) {
        g1z8Total += g1z8Durations[i];
        g1z8Min = std::min(g1z8Min, g1z8Durations[i]);
        g1z8Max = std::max(g1z8Max, g1z8Durations[i]);
    }
    
    for (int i = 0; i < gemmDurations.size(); i++) {
        gemmTotal += gemmDurations[i];
        gemmMin = std::min(gemmMin, gemmDurations[i]);
        gemmMax = std::max(gemmMax, gemmDurations[i]);
    }
    
    int numActualRuns = numRuns; // 排除预热运行
    double g1z8Avg = static_cast<double>(g1z8Total) / numActualRuns;
    double gemmAvg = static_cast<double>(gemmTotal) / numActualRuns;
    
    // 打印结果
    std::cout << "\nFP16精度性能测试结果 (" << numActualRuns << "次运行):" << std::endl;
    std::cout << "conv1x1_g1z8_0626 - 平均: " << g1z8Avg << " μs, 最小: " << g1z8Min
              << " μs, 最大: " << g1z8Max << " μs" << std::endl;
    std::cout << "conv1x1_gemm_32x16_sg - 平均: " << gemmAvg << " μs, 最小: " << gemmMin
              << " μs, 最大: " << gemmMax << " μs" << std::endl;
